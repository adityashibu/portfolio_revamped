    ...
    const fetchRepoCount = async () => {
      try {
        ...
        if (
          ...
          data.total_private_repos !== undefined
        ) {
      ...
      try {
        ...
        const prPromises = repos.map(async (repo) => {
          const prsResponse = await fetch(
            `https://api.github.com/repos/${repo.owner.login}/${repo.name}/pulls?state=all`,
            {
              headers: {
                Authorization: `token ${token}`,
              },
        ...
        const totalCreated = prCounts.reduce(
          (acc, count) => acc + count.created,
          0
        );
        const totalMerged = prCounts.reduce(
          (acc, count) => acc + count.merged,
          0
        );
